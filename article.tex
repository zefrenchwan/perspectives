...
Mathématiquement, l'espace social n'est pas un simple espace affine euclidien inerte. Il est structuré par des contraintes locales. Nous le modélisons comme une variété $\mathcal{M}$ munie d'un \textbf{champ de tenseurs de structure} $\mathbb{L}$.
Pour tout état $E \in \mathcal{M}$, $\mathbb{L}(E)$ est un endomorphisme de l'espace tangent (une transformation linéaire des vecteurs vitesse, ici l'adaptation). Il agit comme une métrique de perméabilité sociale :
\begin{itemize}
	\item Si $\mathbb{L}(E)$ est l'identité, le mouvement est libre.
	\item Si $\mathbb{L}(E)$ possède des valeurs propres nulles, certaines directions sociales sont verrouillées par le système.
	\item Si $\mathbb{L}(E)$ déforme les vecteurs, le système "dévie" les intentions des acteurs vers des issues conformes à la structure sociale.
\end{itemize}
