...
\paragraph{La notion d'espace affine} Un espace affine permet de modéliser des positions (l'état) et des déplacements (l'adaptation). Si on est à la position $p_t$ à $t$ et qu'on se déplace de $dp_t$, la nouvelle position $p_{t+dt} = p_t + dp_t$. Par exemple, dans un espace affine dont les axes sont colère, peur et espoir, si on est au point $p = (0,2,3)$, qu'on réalise un déplacement $dp = (0.5,0.6,0.1)$, on se retrouve à la position $(0.5,2.6,3.1)$, soit $0.5$ en colère, $2.6$ en peur, et $3.1$ en espoir. 


\subsection{La modélisation par états}

L'observateur utilise un espace affine pour modéliser les états des objets $o_i$. Ce point seul mérite qu'on s'y attarde. Dans un système mécanique, la question principale reste la prédiction de la trajectoire d'un objet au fil du temps. L'espace affine en question ne pose pas spécialement de problème quant à sa définition. C'est $\mathbb{R}^2$ pour les mouvements en deux dimensions, et $\mathbb{R}^3$ pour les mouvements en 3D (par définition). Dans un système social, définir ces axes d'analyse est une question tout à fait cruciale : des axes évidents donnent des informations triviales, et de trop nombreux axes induisent un temps de calcul rendant inopérantes les simulations. Mais dès que ces $v$ axes sont connus et "indépendants", ils induisent une base d'un espace affine de dimension $v$. Donnons des exemples concrets: 
\begin{itemize}
...
\end{itemize}
$$\text{ }$$
Notons bien que cette formulation ne dépend que des acteurs et leur interaction. En sociologie, on parle d'\textbf{agency} : ce que ferait un acteur social de manière autonome, sans contrainte de la société. Nous parlerons dans la suite d'adaptation. Notre formulation devient : 
...
\subsection{Prise en compte des facteurs systémiques}
...
\begin{enumerate}
	...
	\item on calcule ce que ferait l'acteur s'il n'était soumis à aucune contrainte, donc son adaptation à la situation : $\mathcal{A}_{it} = \mathcal{S}_{it} \mathcal{R}_{it} + \mathcal{B}_{it}$
	\item à ce stade, on a à la fois l'état courant de l'acteur $E_{it}$, et son effort d'adaptation $A_{it}$. Cet état courant détermine une réaction du système qui s'applique à tous les acteurs ayant ce même état. Le fait que l'on boucle sur les acteurs ne doit pas faire oublier que c'est leur état et pas l'acteur qui induit une limitation $\mathbb{L}(E_{it})$ des états possibles.  Pour la position courante $E_{it}$, la position future corrigée par le système devient $\mathbb{L}(E_{it})(A_{it})$. \textbf{Attention, il s'agit d'une fonction et pas d'une multiplication de matrices}. Dans le cas particulier où $\mathbb{L}(E_{it}$ est une telle matrice (de taille $v \times v$), on notera $\mathbb{L}(E_{it})A_{it}$ (sans parenthèse).  
	\item enfin, on applique une correction mathématique $\sigma$ pour éviter les valeurs aberrantes. Formellement, $\sigma$ est une fonction de $\mathbb{R}^v$ dans $\mathbb{R}^v$. En pratique, on applique une sigmoïde composante à composante. Chaque valeur se retrouve corrigée par une fonction à peu près linéaire pour des valeurs raisonnables, et écrasée les vers un résultat plausible pour les valeurs extrêmes. 
\end{enumerate}

On peut alors regrouper la correction des états (limitation par le système) et la correction du modèle pour les valeurs aberrantes en notant $\Lambda(E_{it}) = \sigma \circ \mathbb{L}(E_{it})$. Finalement, la formule complète devient : 
$$ E_{i,t+dt} = E_{it} + \Lambda(E_{it}) (\mathcal{S}_{it} \mathcal{R}_{it} + \mathcal{B}_{it}) dt$$
...
\hline 
$\Lambda = \sigma \circ \mathbb{L}$ & Fonction définie pour chaque état (pas que l'acteur) \\
  & qui ajuste le comportement de l'acteur en fonction de  \\
 & ce que le système permet (et une correction technique) \\
\hline 
...
\paragraph{Formalisation géométrique du système}
Mathématiquement, l'espace social n'est pas un simple espace affine euclidien inerte. Il est structuré par des contraintes locales. L'usage de variétés est possible, mais elle implique une linéarité qui n'est pas nécessairement souhaitée. A minima, l'espace social est un espace affine qui est muni, en chaque point $p$, d'une fonction $\Lambda_p : \mathbb{R}^v \rightarrow \mathbb{R}^v$. Il devient un espace fonctionnel transformant une impulsion en un vecteur autorisé. 

\subsection{Les mécanismes d'apprentissage}

Un acteur ne subit pas les changements sans réaction : il fait preuve d'adaptation. Au fur et à mesure qu'il évolue, ses réactions (au sens de $dE_{it}$) vont tenir compte de l'importance à traiter telle pression extérieure avec plus ou moins d'urgence ou d'importante. Formellement, c'est sa matrice de sensibilité qui va varier au fil du temps. Ainsi, si l'on inclut des mécanismes de rétroaction de l'acteur, ils portent sur $dS_{it}$. Mais de quoi dépend t'elle ? De la variation d'état $dE_{it}$ bien évidemment, mais aussi de $\mathcal{R}_{it}$ par construction. Autrement dit, l'apprentissage individuel s'écrit $$dS_{it} = \mathbb{A}(dE_{it},\mathcal{R}_{it})dt$$
...
$$\displaystyle { S_{i,t+dt} = S_{it} + \eta ( dE_{it} \otimes \mathcal{R}_{it} )} $$
Soient $dS_{it} = [\theta_{np}]_{1 \le n,p \le v}$, $\mathcal{R}_{it} = [r_n]_{1 \le n \le v}$ le vecteur colonne résultante et $dE_{it} = [\epsilon_n]_{1 \le n \le v}$ le vecteur colonne variation d'état. Alors, pour $n$ et $p$ fixés, il vient 
$$\theta_{np} = \eta \times \epsilon_n  \times r_p $$
...
\begin{enumerate}
	...
	\item sa variation $\theta_{np}$ est ce qu'on va ajouter à $s_{np}$ pour obtenir la nouvelle matrice de sensibilité
	...
	\item $r_p$ qui est la résultante des événements sur l'axe $p$. Plus elle est forte sur cet axe, plus grand est l'impact sur $\theta_{np}$
\end{enumerate}

\subsection{Trajectoires de groupes}

La formule $ dE_{it} = \Lambda(E_{it}) (\mathcal{S}_{it} \mathcal{R}_{it} + \mathcal{B}_{it}) dt$ peut s'interpréter non plus dans l'absolu mais dans une logique de groupe. D'abord, si l'on définit un tel groupe comme une communauté partageant des valeurs et des objectifs communs. On peut alors écrire : 
\begin{enumerate}
	\item que le but $B_{it}$ est en fait centré sur le but du groupe. Autrement dit, le but est partagé et devient indépendant de son porteur. Ce n'est pas exact, bien sûr, mais disons que si $B_t$ désigne le but du groupe $G$ au temps $t$, on peut postuler que $$\mathbb{E}(B_{it})_{i \in G} = B_t$$ avec éventuellement l'introduction d'un bruit gaussien. Autrement dit, la moyenne des buts des acteurs du groupe est le but du groupe. 
	\item que la pression intra groupe est forte, et probablement beaucoup plus forte qu'à l'échelle d'une société entière. On mettrait en place un idéal de groupe à un instant $t$ que chaque membre serait incité à atteindre. Dès lors, $\mathbb{L}(E_{it})$ serait une force de rappel ramenant d'autant plus fort les états les plus éloignés de l'idéal. Par construction même, l'écart type intra groupe des états serait évidemment petit. 
\end{enumerate}

Le principe est donc de dire que $\Lambda$ devient une correction dépendante d'un groupe de référence et pas seulement d'un système entier. Une hiérarchie des normes se met alors en place, permettant de prendre en compte d'abord le groupe auquel l'individu préfère appartenir. 
$$\text{ }$$
Une autre façon de penser les groupes serait de lier la déformation induite par $\Lambda$ à une forme de masse des individus de même état. Très simplement, plus il y a d'individus autour d'un état commun, plus cet état commun créé une courbure dans l'espace des états, plus il attire et plus il est difficile d'en sortir. Un état fortement partagé génère une gravité propre. Cette masse inertielle explique pourquoi les grands groupes survivent aux changements d'humeur individuels : pour déplacer le groupe, il ne faut plus seulement convaincre un individu, mais vaincre la courbure que le groupe impose à son propre espace. 
